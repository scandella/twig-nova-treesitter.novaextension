...
module.exports = grammar({
  ...
  rules: {
    template: ($) =>
      repeat(
        choice($.statement_directive, $.output_directive, $.comment, $.content, $.js_block, $.css_block)
      ),
    ...
    tag_statement: ($) =>
      seq(
        choice(
          ...
          alias('endspaceless', $.keyword),
          alias('endtrans', $.keyword),
          alias('endjs', $.keyword),
    ...
    autoescape_statement: ($) =>
      seq(
        alias('autoescape', $.tag),
        optional(alias($._name, $.attribute))
      ),

    apply_statement: ($) =>
      seq(
        alias('apply', $.tag),
        alias($._name, $.filter_identifier)
      ),
    ...
    js_block: ($) =>
      seq(
        seq(choice('{%', '{%-', '{%~'), alias('js', $.tag), choice('%}', '-%}', '~%}')),
        field('content', $.js_content),
        seq(choice('{%', '{%-', '{%~'), alias('endjs', $.keyword), choice('%}', '-%}', '~%}'))
      ),

    css_block: ($) =>
      seq(
        seq(choice('{%', '{%-', '{%~'), alias('css', $.tag), choice('%}', '-%}', '~%}')),
        field('content', $.css_content),
        seq(choice('{%', '{%-', '{%~'), alias('endcss', $.keyword), choice('%}', '-%}', '~%}'))
      ),
